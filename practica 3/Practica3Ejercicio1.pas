...
begin 
  ...
  while(res = 'si')do begin 
    ...
    ingresar_datos(e1);
    encontrado:=false;
    while(not eof (aEmpleado))and not encontrado do begin 
      read(aEmpleado, e2);
...
begin 
  ...
  while(seguir = 'si')do begin
    ...
    writeln(' h: eliminar un empleado');
    readln(res);
    case res of
      'a': buscar (aEmpleado);
      'b': listar (aEmpleado);
...
procedure leer(var aEmpleado: archivo_empleado;var act: empleado);
begin 
  if(not eof(aEmpleado))then begin
    read(aEmpleado,act);
  end
  else begin 
    act.num_emp := valoralto;
  end;
end;

procedure eliminar(var aEmpleado: archivo_empleado);
var 
  num: integer;
  act: empleado;
  pos: integer;
begin
  ...
  reset(aEmpleado);
  leer(aEmpleado,act);
  while((act.num_emp <> valoralto) and (act.num_emp <> num)) do begin 
    leer(aEmpleado,act);
  end;
  pos := filePos(aEmpleado)-1;
  seek(aEmpleado, filesize(aEmpleado)-1);
  leer(aEmpleado,act);
  seek(aEmpleado,pos);
  write(aEmpleado,act);
  seek(aEmpleado, filesize(aEmpleado)-1);
  truncate(aEmpleado);// borra desde donde este el puntero hasta el final
  close(aEmpleado);
end;
